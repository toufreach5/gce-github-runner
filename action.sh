...
function start_vm {
  ...
  startup_script="
	...
	RUNNER_ALLOW_RUNASROOT=1 ./config.sh --url https://github.com/ReachFive/${GITHUB_REPOSITORY} --token ${RUNNER_TOKEN} --labels ${VM_ID} --unattended ${ephemeral_flag} --disableupdate && \\
	./svc.sh install && \\
	./svc.sh start && \\

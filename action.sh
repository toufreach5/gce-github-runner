...
function start_vm {
  ...
  VM_ID="gce-gh-runner-${GITHUB_REPOSITORY##*/}-$(echo ${GITHUB_SHA}| cut -c1-7)-${GITHUB_RUN_NUMBER}"
  ...
  infra_startup_script="
	...
	RUNNER_ALLOW_RUNASROOT=1 ./config.sh --url https://github.com/${GITHUB_REPOSITORY} --token ${RUNNER_TOKEN} --labels ${VM_ID} --unattended ${ephemeral_flag} --runnergroup default --disableupdate && \\
	./svc.sh install && \\
	./svc.sh start && \\
	gcloud compute instances add-labels ${VM_ID} --zone=${machine_zone} --labels=gh_ready=1
	# 2 days represents the max workflow runtime. This will shutdown the instance if everything else fails.
	nohup sh -c \"sleep 45m && gcloud --quiet compute instances delete ${VM_ID} --zone=${machine_zone}\" > /dev/null &
  "
